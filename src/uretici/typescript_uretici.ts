 ...
 * TypeScript Kod Uretici (Strict Typed)
 */

import { ara_proje_temsili, ara_komut, ara_ifade } from './ara_temsil';

export function typescript_kodu_uret(ara_temsil: ara_proje_temsili): string {
  const satirlar: string[] = [];

  satirlar.push('/**');
  satirlar.push(` * NODEFORGE Visual Programming Studio`);
  satirlar.push(` * Proje: ${ara_temsil.proje_adi}`);
  satirlar.push(` * Hedef Dil: TypeScript (Strict)`);
  satirlar.push(` * Developed By K7~`);
  satirlar.push(` * Üretim Zamanı: ${new Date().toLocaleString()}`);
  satirlar.push(' */');
  satirlar.push('');

  // 1. Tip Dönüşüm Yardımcısı
  function ts_tipi(tip: string): string {
    switch (tip) {
      case 'sayi': return 'number';
      case 'metin': return 'string';
      case 'mantiksal': return 'boolean';
      case 'dizi': return 'any[]';
      case 'nesne': return 'Record<string, any>';
      default: return 'any';
    }
  }
  ...
  if (ara_temsil.degisken_bildirimleri.length > 0) {
    ...
    ara_temsil.degisken_bildirimleri.forEach(deg => {
      const tip = ts_tipi(deg.tip);
      const deger_metni = JSON.stringify(deg.ilk_deger !== undefined ? deg.ilk_deger : null);
      satirlar.push(`let ${deg.ad}: ${tip} = ${deger_metni};${deg.aciklama ? ` // ${deg.aciklama}` : ''}`);
    });
    satirlar.push('');
  }
  ...
  if (ara_temsil.fonksiyon_bildirimleri.length > 0) {
    ...
    ara_temsil.fonksiyon_bildirimleri.forEach(fn => {
      const params = fn.parametreler?.map(p => `${p.ad}: any`).join(', ') || '';
      satirlar.push(`function ${fn.fonksiyon_adi}(${params}): any {`);
  ...
  satirlar.push('async function main(): Promise<void> {');
  satirlar.push('  console.log("[NODEFORGE] Program başlatıldı (TypeScript).");');
  ...
  satirlar.push('}');
  satirlar.push('');
  satirlar.push('// Programı başlat');
  satirlar.push('main().catch(err => console.error("[HATA]", err));');
